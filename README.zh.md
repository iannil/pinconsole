...
> **你的访客，你的数据。** · [English](./README.md) · [官网](https://pinconsole.com) · [博客](https://pinconsole.com/blog/)
...
# 启动基础设施（PostgreSQL + Redis + MinIO）+ 构建 release 二进制
...
| Dashboard — 实时访客监控 | Co-browsing — 运营协同 |
|---|---|
| ![Dashboard](./marketing/public/screenshots/dashboard.webp) | ![Co-browsing](./marketing/public/screenshots/cobrowse-active.webp) |

| 会话回放 | 即时聊天 + 弹窗 |
|---|---|
| ![回放](./marketing/public/screenshots/replay.webp) | ![聊天](./marketing/public/screenshots/chat.webp) |

| 隐私与知情同意控制 |
|---|
| ![隐私](./marketing/public/screenshots/privacy.webp) |
...
```mermaid
flowchart LR
    SDK["访客 SDK<br/>(~15KB gzip)"] -->|rrweb 事件 / ws| Server["Go 服务器<br/>Gin + WebSocket"]
    Admin["管理后台<br/>(Vue 3 SPA)"] <-->|ws + REST| Server
    Server --> PG[("PostgreSQL<br/>元数据")]
    Server --> Redis[("Redis<br/>在线状态 / 锁")]
    Server --> MinIO[("MinIO<br/>事件存储")]
...
**Hub-and-spoke** —— 所有流量经中心服务器。单二进制（Go embed），无外部 CDN，无第三方加载器。
...
| 原则 | 为什么 |
|---|---|
| **数据主权** | 所有访客行为、运营对话、录像存在**你自己的** PostgreSQL / Redis / MinIO。无第三方调用、无外部依赖。 |
...
1. **单实例 hub（不支持横向扩展）** —— WebSocket 路由基于进程内 `map`（`server/internal/hub/hub.go`）。多实例部署（2+ server behind LB）会静默失败——访客和运营连到不同实例后互不可见。横向扩展需引入 Redis Pub/Sub 或 NATS 作为消息总线。

2. **500 WS/房间并发未压测** —— PLAN.md 把"500 WS/房间"作为设计目标驱动决策，但 v1 **未做实际压测**。默认 `PG_MAX_CONNS=25` / `REDIS_POOL_SIZE=50` 是经验值，实际容量需部署方验证。

3. **OSS 项目不提供生产拓扑** —— docker-compose `prod` profile 仅作参考。生产拓扑（VM / k8s / 反代 / TLS / 备份 / 监控 / 日志聚合 / 资源限制）由部署方自行决定。本仓库保证：可重复的 dev/CI 路径、fail-secure release 二进制、`/healthz` + `/readyz` 健康检查。

4. **Trace_id 端到端传播已补全** —— operator browser → server → SDK → server → operator 形成 trace_id 闭环。
...
1. 阅读 [`docs/project-status.md`](./docs/project-status.md) —— 项目的状态和架构唯一事实来源。
2. 查看开放的 [Issues](https://github.com/iannil/pinconsole/issues) —— 标签为 `good first issue` 的是好的起点。
3. 开始重要工作前先开一个 [Discussion](https://github.com/iannil/pinconsole/discussions) —— 先对齐再动手。
4. 遵循 [`CLAUDE.md`](./CLAUDE.md) 中的提交约定 —— 清晰、小巧、单一目的的提交。

**所有贡献默认以 AGPL-3.0 许可授权。** 详见 [`CONTRIBUTING.md`](./CONTRIBUTING.md)（即将推出）。
