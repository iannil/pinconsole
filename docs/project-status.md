...
**v1 全切片(1a-1j)代码已交付 + B 级 reality check 完成(2026-06-18)**

reality check 结果:静态/单测/e2e(39/39)全绿,但切片验证深度不均匀:

- 🟢 verified-deep ×7(1a-1g):端到端深度验证
- 🟡 verified-shallow ×2(1h/1i):e2e 通过但浅
- 🔴 implemented-unverified ×1(1j):代码已交付,无专属 e2e
...
| [1h](./reports/completed/2026-06-17-slice-1h-implementation.md) | 认证 + 多运营 | 🟡 | e2e 浅(API-only + 静默跳过) |
| [1i](./reports/completed/2026-06-17-slice-1i-implementation.md) | 反爬虫 | 🟡 | e2e 浅(rate limit 未真触发) |
| [1j](./reports/completed/2026-06-17-slice-1j-implementation.md) | i18n + 部署 + CI | 🔴 | 代码已交付,无专属 e2e |
...
- **🟡 1h 认证测试浅**:登录/登出/Claim 测试只验证 API 端点存在,未验证 cookie 真设置/真清除/受保护端点真拒绝匿名
- **🟡 1i 反爬虫测试浅**:rate limit 在 prod 模式才生效,e2e 在 dev 模式跑只验证 middleware 注册,未验证 429 真触发
- **🟡 HeadlessChrome UA ban 是死代码**:新版 Playwright chromium UA 是 `Chrome/...` 不含 `HeadlessChrome`,实际绕过
- **🔴 1j 完全无 e2e**:i18n 切换/docker-prod/CI 触发均未验证
...
- ⏳ e2e 文件按切片拆分(realtime.spec.ts 824 行 → 9 个文件)
- ⏳ 补全 1c-1j daily 笔记 retroactive 总结
- ⏳ 更新 MEMORY.md(当前阶段从 Pre-code 改为 v1 delivered)

### A 浅测补深(B 完成后启动)

按 R2 rubric 升级 1h/1i/1j:

- **1h 🟡→🟢**:
  - 加 UI 登录流 e2e(admin SPA 真渲染表单 → 提交 → 跳转)
  - 加未登录访问受保护端点 → 401 e2e
  - 去掉 `if (!sessions.sessions.length) return` 静默跳过,改为预 seed 访客或 fail-fast
  - 加登出后 cookie 真清除验证
- **1i 🟡→🟢**:
  - 加 prod 模式跑 100+ 请求验证 429 触发
  - 加 fingerprint 写 PG 验证(SELECT visitors WHERE fingerprint IS NOT NULL)
  - 加 FlagSession → Redis 验证(GET flagged:session:{id})
  - 修 `HeadlessChrome` UA ban:加 `Chrome/` + headless 指纹检测(behavior-based)或 playwright-特定 marker
- **1j 🔴→🟢**:
  - 加 i18n 切换 e2e(中→英 / 英→中)
  - 加 docker-prod 启动 e2e(docker compose --profile prod up → 等待 ready → curl healthz)
  - 加 CI 触发验证(检查 .github/workflows/ 是否在 PR 时跑通)
