...
**最后更新**：2026-06-20(1ai-g/1ai-h 完成 + marketing-monitor→pinconsole 全量重命名 5 步 + 文档整理归一)
...
### 当前阶段
**v1 主干完全收口 + 测试信心补全 + 1ai-h 接口化重构完成 + 重命名归一(2026-06-20)**。90+ commits 交付:1a-1z 全切片 + 1k-1z 加固 + 1aa-1ai-h 测试深化与接口化重构 + e2e acceptance(65 passed / 0 failed / 4 skipped)+ 5 个 e2e 后真实使用发现的生产 bug fix + admin flagged UI + prod-mode/docker-prod e2e CI + marketing-monitor → pinconsole 全量重命名(5 commit)。
...
- 累计 +75 测试,api 包覆盖实测 28.7%(commit 自报 38.2% 是更高口径),storage 包实测 1.5%(包级,repo 文件级覆盖更高)
