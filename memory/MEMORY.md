...
**最后更新**：2026-06-30(grill-with-docs 审计第二轮 → CLAUDE.md 8 处事实源冲突修复;第一轮 marketing/ license 拆分 ADR-0001 + CONTEXT.md 创建)
...
### 项目定位
构建某商业竞品的**开源替代品**。竞品是 ToB 实时监控 + 互动客服 / 营销转化平台。仓库分两层(详见 [`CONTEXT.md`](../CONTEXT.md) + [`docs/adr/0001-license-split.md`](../docs/adr/0001-license-split.md)):
- **OSS Core**(AGPL-3.0-or-later):专注技术核心,不做客户获取与销售(不做计费/注册/多租户 SaaS)
- **Marketing Layer**(`marketing/`,UNLICENSED):maintainer 咨询转化官网,与 OSS 物理隔离

### 当前阶段
**v1 已发布 (v0.1.2, 2026-06-23) + post-v1 已完成 Widget Config Editor + 自定义域名 + vendor-rrweb + grill-with-docs 审计收口 (2026-06-30)**

- **v1 主干 + 加固 + 测试深化** 全部合并(1a-1j + 1k-1z + 1aa-1ai-h, 90+ commits, 2026-06-17~20)
- **vendor-rrweb 硬分叉**: alpha.20 → `packages/replay-core` 自维护, 删 Svelte rrweb-player (~600 行 hack), nodeID 跨端寻址打通 (2026-06-25~26, 21 commits, `82a7a355`)
- **Go 后端覆盖**: recording 90.7% / api 90.0% / storage 91.5% (2026-06-26 实测)
- **Widget Config Editor** (pe-1/2/3): 运营表单式编辑 popup/chat/banner/consent 文案 → PG `widget_configs` → SDK 配置驱动渲染 (2026-06-26, commits `19eca36`/`42a5625`/`1dcda74`)
- **自定义域名** (cd-1/cd-2): certmagic + Let's Encrypt ACME + HostDomainMiddleware + migration 000009 + Admin DomainsView (2026-06-26~27)
- **端口迁移**: 开发端口全部移至 7000-7100 范围 (7080/7073/7074/7032/7079/7020/7021, 修复 macOS AirPlay 冲突)
...
- **grill-with-docs 审计** (2026-06-30): 锁定 OSS Core / Marketing Layer license 拆分 (ADR-0001) + 创建 CONTEXT.md 项目术语表 + 修复 CLAUDE.md 8 处事实源冲突 (commits `d65230b`/`86091f6`)

详见 [`docs/project-status.md`](../docs/project-status.md) §2.1/§5。

**下一步候选**(post-v1 backlog,详见 [`PLAN.md`](../PLAN.md) §8):
- Tauri 桌面端 (Win + Mac, 复用 admin SPA)
- Page Editor (拖拽 landing 编辑器, 注意 ≠ Widget Config Editor)
- 反爬加固 (CAPTCHA + honeypot + 动态类名/ID)
- fork-3b 上游测试转 Playwright (5 组, ~2-3d)
- T2/T3 测试 backlog (~40 项, ~15h)

### 范围边界
- **不做(OSS Core,永久)**:多租户 SaaS、计费、注册流、云托管服务
- **已完成 post-v1**:Widget Config Editor(pe-1/2/3)、自定义域名(cd-1/cd-2)、vendor-rrweb
- **未做(post-v1 backlog)**:Tauri、Page Editor(拖拽 landing 编辑器)、反爬加固、SSO
...
- License:**OSS Core (AGPL-3.0-or-later) + Marketing Layer (UNLICENSED)** 拆分,详见 ADR-0001
- 仓库:Monorepo,Go embed 静态资源(admin/sdk/landing);`marketing/` 不进 embed,独立 Cloudflare 部署
- 后端:Go + Gin + coder/websocket + 自定义 hub
- 存储:PostgreSQL + Redis + MinIO
- 前端:Vue 3 + TS + Vite + Pinia + Vue I18n(中英 day 1;**Element Plus 已于 1q 移除**,手撸样式 + Calm Crafted 设计 token)
- 管道:中心化 hub-and-spoke(无 WebRTC、P2P)
- 租户:单租户 + schema 预留

### v1 切片技术

- SDK:rrweb 全量采集(vendor-rrweb 硬分叉,`packages/replay-core`)+ 提交前按键 + 选择性截图(canvas/iframe)
- co-browsing:rrweb 双向 + 节点 ID 选择器 + 防抖 300ms 代填
- 多运营:1:1 锁定
- 认证:Email/password + bcrypt + HttpOnly cookie
- 域名:**v1 平台域名 + post-v1 自定义域名(cd-1/cd-2 已完成,certmagic + ACME)**
...
### Maintainer 营销层(2026-06-22 17 轮 grill-me 共识 + 2026-06-30 license 拆分)

详见 [`docs/progress/2026-06-22-landing-readme-design.md`](../docs/progress/2026-06-22-landing-readme-design.md) + [`docs/adr/0001-license-split.md`](../docs/adr/0001-license-split.md)。索引：

- **策略**：v1 完成后，maintainer 接受 ToB 咨询（PLG + 咨询式）。产品本身仍 OSS 自托管 / AGPL / 不计费 / 不做注册流
- **License 拆分**（2026-06-30 锁定）：`marketing/` 从 AGPL 改为 **UNLICENSED / All Rights Reserved**，与 OSS Core 物理隔离 + 法律隔离。理由：AGPL share-alike 会强制 maintainer 写的营销文案/客户案例公开可被竞品 fork
...
- **IA**：单页长滚动 8 段漏斗型（Hero/Problem/Features/数据主权/Self-host/Roadmap/FAQ/FinalCTA）+ 独立 blog + use-cases + alternatives/竞品对比 SEO 页（2026-06-29 扩展）
- **分析**：保留 Google Analytics（PUBLIC_GA_MEASUREMENT_ID = `G-ELP9L7M8GB`），接受与 privacy-first 产品本体的品牌张力（2026-06-30 决策）
- **视觉**：Calm Crafted 营销变体（复用 docs/design-system.md `--pc-*` tokens，加大字号 hero + 更宽留白 + Gentle & Restrained 动效）
- **技术栈**：Astro + Vue → Cloudflare Pages + Workers + D1 + Resend
- **仓库结构**：`marketing/` 独立目录,license 边界由 `marketing/LICENSE` + `marketing/package.json` `private: true` + `license: UNLICENSED` 标记

**Phase 进度**：
- ✅ Phase 1~3：marketing/ Astro 骨架 + Calm Crafted 营销变体 tokens + 中英双语路由 + 表单/Worker/D1 schema + 5 张真实截图 + 端到端验证(2026-06-22)
- ✅ Phase 4：README 顶部 + PLAN.md §1 + MEMORY.md 同步(2026-06-22)
- ✅ Phase 5：blog / use-cases / alternatives SEO/GEO 扩展 + RSS + llms.txt + GA + Turnstile + Resend(2026-06-29)
- ✅ Phase 6：license 拆分(`AGPL` → `UNLICENSED` + ADR-0001)(2026-06-30)

### 工作流

- 文档分层:根(`PLAN.md` / `CONTEXT.md` / `CLAUDE.md` — 事实来源) + `docs/`(工作文档 + `adr/` + `reports/`) + `memory/`(记忆)
- 切片序:v1 阶段是 1a→1b→...→1j 单序列不跳步;**post-v1 多线索并行**(Widget Config / 自定义域名 / vendor-rrweb 各自独立),"不跳步"规则仅适用于单序列内
- 进展即记录:每次修改都有 progress/completed 文档
- 审计节奏:重大节点(grill-me / 接手项目 / 架构变更)用 grill-with-docs 走结构化访谈 + ADR + CONTEXT.md 更新
...
**反 AI-slop 红线**(写入 docs/design-system.md §1.2):禁紫渐变 / Inter/Geist/Roboto / slate+indigo / emoji-as-icon / 单层 box-shadow / 首屏 stagger 揭示。

**实现状态**:已落地。admin SPA 全量重写为 Calm Crafted 设计(2026-06-21 后多切片渐进迁移),visitor-sdk 共享 `--pinconsole-*` token 前缀。
...
### CLAUDE.md 漂移检测必须定期做(2026-06-30)

**Why**:CLAUDE.md 是**每次 Claude 会话注入 system prompt**的高优先级文件。项目演进时,CLAUDE.md 中关于"当前阶段、已用技术栈、已完成切片、license 策略"的硬性陈述若不与代码同步,会**系统性误导后续所有 Claude 实例**——而后续实例难以发现(它们读到的是"事实")。

2026-06-30 grill-with-docs 审计第二轮在 CLAUDE.md 发现 **8 处与事实源冲突**:
- L7 "不做营销页" + License:AGPL-3.0(与 ADR-0001 license 拆分冲突)
- L102 事实源只列 PLAN.md + 本文件(漏 CONTEXT.md / project-status.md / docs/adr/)
- L108 "范围:v1...不含页面编辑器、自定义域名"(均已完成)
- L114 前端含 "Element Plus"(2026-06-18 已移除)
- L121 "v1 仅平台域名,自定义域名是后续切片"(cd-1/cd-2 已完成)
- L126 "v1 切片拆为 1a-1j 共 10 个子切片"(实际 60+)
- L131 "v1 之外的能力(页面编辑器、Tauri、自定义域名、多租户)"(前两项已完成)
- L138 "AGPL-3.0 一等公民"(应改述 license 拆分)

**根因**:重大架构变更(Element Plus 移除、自定义域名完成、marketing/ 上线、Widget Config Editor 完成)发生时,只更新了 PLAN.md / project-status.md / MEMORY.md,但**漏了 CLAUDE.md**——因为 CLAUDE.md 不在切片完成报告的"必更新"清单里。

**How to apply**:
1. **每个 post-v1 切片完成时**,在 impl 报告的"文档同步清单"中加入 **CLAUDE.md 检查**——逐项 grep CLAUDE.md 中提及本切片的硬性陈述
2. **架构变更(license / 框架 / 仓库布局)时**,CLAUDE.md 必须与 PLAN.md 同步更新,且作为 PR review 的强制项
3. **接手项目做 reality check 时**,CLAUDE.md 与代码的对照应作为第 0 步(早于读 PLAN.md),因为 CLAUDE.md 是注入的,可信度被默认设为最高,但实际上最容易漂移
4. **凡 CLAUDE.md 出现"v1 不做 X / X 不存在 / X 是后续切片"等将来时陈述,3 个月后必须重新验证**(漂移高频区)

详见 [`memory/daily/2026-06-30.md`](./daily/2026-06-30.md) 第二轮审计段。
